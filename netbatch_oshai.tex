...
\usepackage{url}
\urldef{\mailsa}\path|{ohad.shai, edi.shmueli}@intel.com|
\urldef{\mailsb}\path|feit@cs.huji.ac.il|
...
\title{Heuristics for Resource Matching\\in Intel's Compute Farm}
...
Prof. Dror G. Feitelson And Prof. Sivan Toledo
Tel Aviv University
Jun 2014
...
%MATAM Industrial Park, Haifa, ISRAEL\\
%\mailsa\\
...
%Currently used heuristics for mapping jobs to machines include \bef\ and \wof, applied either to the cores resource or to the memory resource.
%We show that both these heuristics may fail to properly utilize the nodes, because one resource may be depleted much before the other.

%We therefore propose \mif, which is guided by the goal to utilize both resources in a balanced manner.
%Simulations based on real workload traces from a large cluster indicate that \mif\ outperforms other heuristics 75--80\% of the time, and that under heavy loads it can reduce average wait times by as much as 15\%.
%These results are similar to those of a meta-heuristic that employs all the other heuristics as sub-routines, and at each scheduling step selects the one that manages to allocate the most jobs.

\keywords{\nb, Job Scheduling, Resource Matching, Simulation, \bef, \wof, \fif}
\end{abstract}
...
\section{Matching Machines to Jobs}
%==================================
\label{sec:matching}
...
\subsection{Synthetic Examples of Heuristics Failures}
%-----------------------------------------------------
...
\subsection{Observations from the Workloads}
%-------------------------------------------
...
\subsection{Comparing Heuristics}\label{sec:buckets}
%--------------------------------
...
\subsection{Balanced Resource Usage}
%-----------------------------------
...
\section{The \maj\ Meta-Heuristic}
%=================================
\label{sec:max-jobs}
...
matching.
In this paper we investigated the problem of resource matching in
